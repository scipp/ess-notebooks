if __name__ == '__main__':
    ...
    plt.plot(z, trace(z, 0))
    plt.plot(z, z * dtrace(0, 0))
    plt.plot(z, np.linspace(0.00001, trace(z, 0)[-1], 1000), ls='--')
    plt.xlim(-1, 4.8)
    plt.ylim(-0.3, 1.5)
    rectangle = plt.Rectangle((-0.75,-0.25), 1.5, 0.25, ec='k', fc=(1, 0, 0, 0))
    plt.gca().add_patch(rectangle)
    rectangle = plt.Rectangle((z1,0.3), 0.4, 1, ec='k', fc=(1, 0, 0, 0))
    plt.text(0, -0.125, 'Sample', horizontalalignment='center', verticalalignment='center')
    plt.text(z1+0.2, 0.8, 'Detector', horizontalalignment='center', verticalalignment='center', rotation='vertical')
    plt.gca().add_patch(rectangle)
    plt.xlabel('$z$/m')
    plt.ylabel('$y$/m')
