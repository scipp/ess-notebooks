...
class monoGaussCoil(IFunction1D):
    ...
    def _monoGaussCoil_core(self, qval):
        ...
        if qr < 0.8:
            ...
            C7 = -1. / 181440.
            return (((((
                (C7 * qr + C6) * qr + C5) * qr + C4) * qr + C3) * qr + C2) * qr
                    + C1) * qr + C0
    ...
    def _first_derivative_rg(self, qval):
        ...
        if qr < 0.8:
            ...
            C6 = +6. / 20160.
            C7 = -7. / 181440.
            return 2.0 * qval * qval * Rg * (((((
                (C7 * qr + C6) * qr + C5) * qr + C4) * qr + C3) * qr + C2) * qr
                                             + C1)

        return 4.0 * qval * qval * Rg * ((math.exp(-qr) - 1.0) *
                                         (-2.0 / qr - 1.0) /
                                         (qr * qr) - 2.0 / qr)
...
class polyGaussCoil(IFunction1D):
    ...
    def _polyGaussCoil_core(self, Rg, polydispersity, q):
        ...
        if polydispersity == 1.0:
            ...
            if z > 1e-04:
                return (2.0 * (np.power(1.0 + u * z, -1.0 / u) + z - 1.0) /
                        (1.0 + u)) / z**2
    ...
    def _first_derivative_rg(self, qval):
        ...
        return I0 * (self._polyGaussCoil_core(Rg_up, polydispersity, qval) -
                     self._polyGaussCoil_core(Rg, polydispersity, qval)) / (
                         Rg_up - Rg)
    ...
    def _first_derivative_polydispersity(self, qval):
        ...
        return I0 * (self._polyGaussCoil_core(Rg, polydispersity_up, qval) -
                     self._polyGaussCoil_core(Rg, polydispersity, qval)) / (
                         polydispersity_up - polydispersity)
    ...
    def function1D(self, xvals):
        ...
        output = np.zeros(len(xvals), dtype=float)
        for i, x in enumerate(xvals):
            output[i] = bgd + I0 * self._polyGaussCoil_core(
                Rg, polydispersity, x)
        return output
    ...
    def functionDeriv1D(self, xvals, jacobian):
        ...
        for x in xvals:
            jacobian.set(
                i, 0,
                self._polyGaussCoil_core(self.getParameterValue('Rg'),
                                         self.getParameterValue('Mw_by_Mn'),
                                         x))
            jacobian.set(i, 1, self._first_derivative_rg(x))
